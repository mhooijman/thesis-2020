...
def inter_intgrads(input_value, reference_value, grads_and_activation_func, 
    ...
    while intermediate_values:
        print(f'Compute gradients for intermediate values {i}')
        i += 1
...
def neuron_importance(input_dir, output_dir, riemann_steps):
    ...
    with tfv1.Session(config=Config.session_config) as session:
        ...
        print(f'{len(input_files)} audio files found. Start computing neuron importance...')
        
        # Calculate neuron scores for each input file
        for input_count, file_name in enumerate(input_files):
            print(file_name)
            if input_count in range(len(input_files), 10): print(f'... {input_count} audio files processed...')
            ...
            print(f'Activations for {file_name} are saved to: {save_to_path_activations}')
